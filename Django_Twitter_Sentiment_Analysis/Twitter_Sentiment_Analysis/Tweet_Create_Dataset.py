...
class Tweet_Create_Dataset:

  def create_training_dataset(train_head_count, test_head_count):   
      try:        
        raw_dataset = pd.read_csv('Twitter_Sentiment_Analysis/DataSet/Sentiment Analysis Dataset.csv', header = 0, error_bad_lines=False)    
        ...
        raw_ds_final_train = Tweet_Create_Dataset.create_pos_neg_dataset(raw_dataset_train, train_head_count)
        raw_ds_final_train = raw_ds_final_train.values 

        raw_ds_final_test = Tweet_Create_Dataset.create_pos_neg_dataset(raw_dataset_train, test_head_count)
  ...
  def fetch_tweet(user_name):
      try:               
         ...
         cofig_file = open("Twitter_Sentiment_Analysis/config.json").read()
         ...
         api = Tweet_Create_Dataset.IniTwitterApi(consumer_key, consumer_secret, access_token, access_token_secret)
